...
namespace function.math
{
	...
	public class FMul : TwoSidedFunction
	{
		...
		public override Function GetSimplifiedFunction()
		{
            ...
            bool checkConsLHS = newLHS.GetID() == FCons.ID;
            bool checkConsRHS = newRHS.GetID() == FCons.ID;

            bool checkZero = (checkConsLHS && newLHS.GetValue() == 0) || (checkConsRHS && newRHS.GetValue() == 0);
            if (checkZero)                  return new FCons(0);
            
            if (checkConsLHS && checkConsRHS)
